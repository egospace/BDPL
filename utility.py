...
def calculate_hit_ndcg(sorted_list,topk,true_items,hit_purchase,ndcg_purchase):
    for i in range(len(topk)):
        ...
        for j in range(len(true_items)):
            # if i == 1:
            #     print(true_items[j], rec_list[j])
            if true_items[j] in rec_list[j] :
                rank = topk[i] - torch.argwhere(rec_list[j] == true_items[j])[0][0]
                hit_purchase[i] += 1.0
...
class CLSTMCell(LayerRNNCell):
  ...
  def call(self, inputs, state):
    ...
    # split concatenated input into item and behavior
    ...
    # new input refers to inputs,m_prev,behaviors
